...
module Output ( M : Ast_or_mlw) = struct
  ...
  let print_metarg fmt m =
    ...
    | Mval qualid -> fprintf fmt (ast_or_mlw "Mval(%a)" "Mval(%a)") print_qualid qualid
  ...
  let [@warning "-39"] rec print_decl fmt d =
    ...
    (* | Dclone(qualid,clone_subst_list) -> fprintf fmt (ast_or_mlw "Dclone(%a,%a)" "Dclone(%a,%a)") print_qualid qualid (Pp.print_list Pp.comma print_clone_subst) clone_subst_list *)
    (* | Duse qualid -> fprintf fmt (ast_or_mlw "Duse(%a)" "@[<hov 2>use %a@]") print_qualid qualid *)
    | Dcloneexport(qualid,clone_subst_list) -> fprintf fmt (ast_or_mlw "Dcloneexport(%a,%a)" "Dcloneexport(%a,%a)") print_qualid qualid (Pp.print_list Pp.comma print_clone_subst) clone_subst_list
    | Duseexport qualid -> fprintf fmt (ast_or_mlw "Duseexport(%a)" "@[<hov 2>use export %a@]") print_qualid qualid
    | Dcloneimport(loc,import,qualid,id_opt,clone_subst_l) ->
        (
          match pp_kind with
    ...
    | Duseimport(loc,import,qualid_id_opt_l) ->
        (
          match pp_kind with
    ...
    | Dimport(qualid) -> fprintf fmt (ast_or_mlw "Dimport(%a)" "@[<hov 2>import %a@]") print_qualid qualid
    | Dscope(loc,import,id,decls) ->
        (
          match pp_kind with
